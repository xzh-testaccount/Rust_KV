...
页面顶部固定展示品牌、执行模式、服务状态、实验导航和当前键总数。指标条只展示当前键总数，不虚构未有来源的系统指标。首次打开默认进入答辩模式：以空视图等待真实后端返回数据，后端不可达时明确显示离线，不先展示前端模拟键值。
...
| 模式 | 页面语义 | 数据与请求规则 |
| --- | --- | --- |
| 退出答辩模式后的纯前端模式 | 纯前端 UI / 动画测试 | 使用浏览器内存和前端状态机；性能实验入口禁用，不生成 QPS 或延迟数据；其他演示数据均标为非实测 |
...
Performance Lab 只允许在答辩模式且真实后端在线时运行。首组准备、`A → B/C` 切换和每次 Retry 前都必须先获得后端 `POST /api/benchmark/reset` 返回的 `ready=true` 和递增 `resetEpoch`，再开始下一组或重试；纯前端模式不播放性能实验、不生成曲线。

切换模式或“重置界面状态”会清理前端实验进度、图表和日志。答辩模式下的重置只清理前端状态，不删除后端数据或修改 WAL。

## 2. 视觉与组件基线

### 2.1 色彩与字体

页面默认使用暗色实验室外壳：`#080b10` 背景、28px 网格、半透明蓝黑面板、1px 边框和克制辉光；用户明确选择浅色后才由 `localStorage` 恢复浅色。新增样式优先使用 CSS token，不在 JSX 中散落新颜色。
...
- 答辩模式使用投屏可读字号：辅助信息不低于约 `10px`，正文约 `11–14px`，关键结论与指标进一步放大；纯前端开发模式仍可使用紧凑布局。
...
参数为客户端数量 `1 / 10 / 50 / 100`（答辩正确性演示默认 10）、每客户端请求数（默认 100）和 `Read / Mixed / Write` 访问模式。活动矩阵、进度环和结果区展示总请求、完成数、Success、Failed。
...
| 档位 | Clients | 采样方式 | 用途 |
| --- | --- | --- | --- |
| 快速演示（默认） | `1 / 128` | 每点固定 3 秒，不按并发数放大总请求量 | 答辩现场实时验证 |
| 完整实验 | `1 / 10 / 50 / 100` | 每轮 10,000 请求，Warmup × 1，Measured × 5 | 报告、正式分析与数据归档 |

两档只改变实验点、采样时长/请求量和采样轮次；数据集、值大小、协议、localhost、随机种子和 `WAL + flush + sync_data` 持久化语义保持一致。页面必须把两档标成“本次实时实验”，不得把历史数据冒充本轮结果。
...
固定条件面板必须展示：`10,000 Keys` 数据集、`128 B` 值大小、当前档位的采样方式、`WAL + sync_data`、`JSON Lines`、`Localhost`。持久化条件不可由实验者切换。

快速档固定按 `1 → 128` 执行；完整档按选中的 `1 → 10 → 50 → 100` 升序执行。多组答辩演示遵循 `A → 重置相同实验环境 → B/C`：切换对照系列前等待后端确认环境重置，并在执行序列中计数 `Environment Resets`。每个点完成后，执行格、吞吐图和延迟图必须立即显示该点真实结果。

#### B 模块提高项与实时 Compact

“基础 WAL vs Snapshot + WAL 压缩”历史实测面板与“Snapshot / WAL 状态与手动压缩”实时存储面板固定位于 Performance Lab 页面最顶部（先于实验参数区与预设区展示），先展示已存档实测基线与当前存储状态，再进入可控变量实验；历史实测不与并发模型的实时曲线混为同一实验：
...
图表、摘要和结论随已完成 Scale 动态更新：答辩模式明确显示“本次实时实验 · 快速演示/完整实验”，只接收后端返回的 QPS、P50、P95、P99、成功数和耗时；纯前端模式保持空图并提示必须连接真实 Rust 后端。单次结论描述最高吞吐；对照结论在同一 Clients 规模比较吞吐与 P99 方向，不预设哪种实现一定更快。
