...
每个版本完整运行三次，下表对耗时采用三次结果的中位数。耗时会受电脑负载、磁盘缓存和硬件影响，文件大小是确定值。
...
| 指标 | 基础版 | 创新版 | 变化 |
| --- | ---: | ---: | ---: |
| 2000次同步写入 | 4787 ms | 4970 ms | 创新版慢约3.8% |
| 未压缩WAL | 170000 B | 238893 B | 创新版大约40.5% |
| 压缩耗时 | 无 | 42167 μs | 约42.2 ms |
| 压缩后WAL | 无压缩 | 0 B | 历史日志已清空 |
| 压缩后快照 | 无 | 4473 B | 保存100个最终键值 |
| 压缩后总持久化大小 | 170000 B | 4473 B | 创新版减少约97.4% |
| 启动恢复中位数 | 2117 μs | 1518 μs | 创新版快约28.3% |

创新版自己的压缩前后对比为 `238893 B → 4473 B`，占用减少约98.1%。

结果说明：序号和版本字段会增加每条日志的体积，并带来少量序列化开销；快照压缩则在历史修改很多、最终有效键较少时显著降低磁盘占用和恢复工作量。这种“明确展示收益和成本”的结果比只报告优化百分比更适合课程答辩。
