//! Command parsing and the JSON Lines wire protocol.
...
/// Maximum JSON payload size in one line, excluding its line terminator.
...
/// A single bounded frame read from a JSON Lines stream.
#[derive(Debug, PartialEq, Eq)]
pub enum Frame {
    /// A complete line, including its LF terminator.
    Line(Vec<u8>),
    /// A complete line exceeded the payload limit and was discarded through LF.
    TooLarge,
    /// The stream reached EOF before reading any bytes for a frame.
    Eof,
    /// The stream reached EOF after reading a non-empty unterminated frame.
    Incomplete,
}

/// Reads one bounded JSON Lines frame without retaining an unbounded line.
...
/// A command accepted by both the text client and the JSON protocol.
...
impl Request {
    /// Validates fields after deserialization or text parsing.
...
/// Details carried by an unsuccessful response.
#[derive(Debug, Clone, PartialEq, Eq, Deserialize, Serialize)]
...
/// One response corresponding to one request line.
#[derive(Debug, Clone, PartialEq, Eq, Deserialize, Serialize)]
...
/// Parses one complete JSON Lines request terminated by `\n` or `\r\n`.
...
/// Parses bytes from a stream, reporting invalid UTF-8 with its stable code.
...
/// Parses a response line received by the client.
...
/// Serializes one request and appends its JSON Lines delimiter.
pub fn encode_request_line(request: &Request) -> Result<Vec<u8>> {
    request.validate()?;
    encode_json_line(request)
}

/// Serializes one response and appends its JSON Lines delimiter.
pub fn encode_response_line(response: &Response) -> Result<Vec<u8>> {
    encode_json_line(response)
}

/// Serializes any protocol value as one bounded JSON Lines frame.
...
/// Parses one text command entered at the client prompt.
...
/// Tokenizes whitespace-separated commands while allowing JSON quoted strings.
